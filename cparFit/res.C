...
TComplex FRL2(double s, double t, double n1, double n2, double L1, double L2) {
  double nL4_1 = n1*L1*L1*L1*L1;
  double nL4_2 = n2*L2*L2*L2*L2;
  double left = TMath::Power(nL4_1 / (nL4_1 + sq(s-sq(Mp))), (Double_t) n1);
  double right = TMath::Power(nL4_2 / (nL4_2 + sq(t)), (Double_t) n2);
...
TComplex I_R(double Eg, double costh, int hel_phi, int mf, TLorentzVector k1, double n1, double n2, double L1, double L2) {
  double s = 2*Mp*Eg + sq(Mp);
  double E_phi = (s + sq(Mphi) - sq(Mp)) / (2*TMath::Sqrt(s));
  double p_phi = TMath::Sqrt(sq(E_phi) - sq(Mphi));
  TLorentzVector k2(p_phi*TMath::Sqrt(1-sq(costh)),0,p_phi*costh,E_phi);
  double t = (k1 - k2).M2();
  return ((I_Rs(Eg,costh,hel_phi,mf,k1) + I_Rt(Eg,costh,hel_phi,mf,k1) + I_Rc(Eg,costh,hel_phi,mf,k1))*FRL2(s,t,n1,n2,L1,L2));
}

TComplex iImI(double Eg, double costh, int hel_gamma, int hel_phi, int mi, int mf, double n1, double n2, double L1, double L2) {
  if (2*hel_gamma+mi != 2*hel_phi+mf) return TComplex(0,0);
  if (Eg<1.7) return TComplex(0,0);
  double W = TMath::Sqrt(2*Mp*Eg + sq(Mp));
...
//  sum *= TComplex(sq(dx),0);
  ...
  if (magic_cos < 1.-dx/2) {
    for (double cosK=magic_cos+dx/2; cosK<1.; cosK+=dx) {
      ...
      sum += I_L(Eg, hel_gamma, mi, pK)*TComplex::Conjugate(I_R(Eg, costh, hel_phi, mf, pK, n1, n2, L1, L2));
    }
    for (double cosK=magic_cos-dx/2; cosK>-1.; cosK-=dx) {
      pK = TLorentzVector(r*TMath::Sqrt(1-sq(cosK)), 0., r*cosK, TMath::Sqrt(sq(r)+sq(Mk)));
      sum += I_L(Eg, hel_gamma, mi, pK)*TComplex::Conjugate(I_R(Eg, costh, hel_phi, mf, pK, n1, n2, L1, L2));
