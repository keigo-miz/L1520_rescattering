...
double rr1(double Eg) {
  return rho0_R(Eg, 1., 0, 0).Re();
}

double rr2(double Eg) {
  return rho0_R(Eg, 1., 1, -1).Re();
}

double rr3(double Eg) {
  return ((rho1_R(Eg,1.,1,-1).Re() - rho2_R(Eg,1.,1,-1).Im())/2.);
}
...
double rr5(double Eg) {
  return (2*rho1_R(Eg,1.,1,1).Re() + rho1_R(Eg,1.,0,0).Re());
}
